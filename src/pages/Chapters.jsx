    ...
    <div className={`${ theme === "dark" ? "bg-gray-800 text-slate-300" : "bg-gray-100 text-slate-800"} grid grid-cols-1 gap-2 p-3 min-h-screen items-center justify-center`}>
      <div>
        <div className='flex text-center cursor-pointer border-x border border-gray-500 rounded-t'>
          <span onClick={()=>handleSortBy('id')} className='border-r border-gray-500 w-full py-2'>Surah {sortType === 'id' && sortOrder ? '⇩' : '⇧'}</span>
          <span onClick={()=>handleSortBy('sajda')} className='w-full py-2'>Sajda {sortType === 'sajda' && sortOrder ? '۩' : '⇧'}</span>
          <span onClick={()=>handleSortBy('words')} className='border-l border-gray-500 w-full py-2'>Word {sortType === 'words' && sortOrder ? '⇩' : '⇧'}</span>
        </div>
