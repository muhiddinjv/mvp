...
import {useChaptersData} from '../hooks';

function Chapters() {
  const { theme } = useTheme("dark");
  const [sortType, setSortType] = React.useState('id');
  const [sortOrder, setSortOrder] = React.useState(false);
  const { setChapterId, setChapters } = useContext(GlobalContext);
  const [,, getParsedBookmarks] = useBookmarks();

  // Load chapters data dynamically from our custom hook.
  const { chapters, loading } = useChaptersData();
  ...
  const handleSortBy = (type) => {
    ...
    // Sort chapters and update context (if needed)
    // Here we sort the local chapters list; if you maintain chapters in context, update accordingly.
  };
    ...
    <div className={`${ theme === "dark" ? "bg-gray-800 text-slate-300" : "bg-gray-100 text-slate-800"} grid grid-cols-1 gap-2 p-3 min-h-screen items-center justify-center`}>
      <div>
        <div className="flex text-center cursor-pointer border-x border border-gray-500 rounded-t">
          ...
          <span onClick={() => handleSortBy('id')} className="border-r border-gray-500 w-full py-2">
            Surah <FontAwesomeIcon icon={sortType === 'id' && sortOrder ? faArrowDown : faArrowUp} />
          </span>
          <span onClick={() => handleSortBy('sajda')} className="w-full py-2">
            Sajda {sortType === 'sajda' && sortOrder ? '۩' : <FontAwesomeIcon icon={faArrowUp} />}
          </span>
          <span onClick={() => handleSortBy('words')} className="border-l border-gray-500 w-full py-2">
            Word <FontAwesomeIcon icon={sortType === 'words' && sortOrder ? faArrowDown : faArrowUp} />
          </span>
          <Link to="/stories" className="border-l border-gray-500 w-full max-w-12 py-2">
            <FontAwesomeIcon icon={faBook} />
          </Link>
        </div>
        <ul className={`${getParsedBookmarks().length === 0 && 'hidden'} border-x border-b border-gray-500 rounded-b p-1 flex justify-center flex-wrap`}>
          {getParsedBookmarks()
      ...
      {chapters.map(chapter => (
        <span id={chapter.id} key={chapter.id} onClick={() => handleChapter(chapter)} className="flex border border-gray-500 rounded">
          <span className="flex items-center justify-center border-r border-gray-500 text-lg w-full max-w-12">
            {chapter.id}
          </span>
          <Link to={`/${chapter.id}`} className="flex-grow p-1 ml-2">
            <div className="font-bold">{chapter.text}</div>
            <div className="text-sm">
              {chapter.verses} verses | {chapter.words} words {chapter.sajda && <span> ۩</span>}
            </div>
          </Link>
          <span onClick={() => alert(`download ${chapter.id}-${chapter.text}'s audio`)} className="w-full max-w-12 border-l border-gray-500 p-2 flex justify-center items-center cursor-pointer">
