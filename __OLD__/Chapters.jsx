...
import { useTheme, useBookmarks } from './hooks/index.jsx';
...
function Chapters() {
  ...
  return (
    <div
      className={`${theme === 'dark' ? 'bg-gray-800 text-slate-300' : 'bg-gray-100 text-slate-800'} grid grid-cols-1 gap-2 p-3 min-h-screen items-center justify-center`}
    >
      <div>
        ...
        <ul
          className={`${bookmarks.length == 0 && 'hidden'} border-x border-b border-gray-500 rounded-b p-1 flex justify-center flex-wrap`}
        >
          {getParsedBookmarks()
            ...
            .map(bookmark => (
              <li
                ...
                className="border border-gray-500 rounded m-1 px-2 py-1 cursor-pointer transition duration-200 ease-in hover:scale-110"
              >
                <Link
                  to={`/${bookmark.chapterId}`}
                  onClick={() => handleBookmark(bookmark)}
                  state={{ fromBookmark: true }}
                >
      ...
      {chapters?.map(chapter => {
        return (
          ...
          >
            ...
            <span
              onClick={() => alert(`download ${chapter.id}-${chapter.text.tr}'s audio`)}
