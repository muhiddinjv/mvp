...
const seedData = async () => {
  ...
  const existingDeck1 = await db.decks.where('name').equals('Greetings').first();
  const existingDeck2 = await db.decks.where('name').equals('Common Phrases').first();
  ...
  let deckId1;
  if (!existingDeck1) {
    deckId1 = await db.decks.add({ name: 'Greetings' });
    ...
    await db.cards.bulkAdd(deck1Cards);
  }

  // Add Deck 2 if it doesn't exist
  let deckId2;
  if (!existingDeck2) {
    deckId2 = await db.decks.add({ name: 'Common Phrases' });
    ...
    await db.cards.bulkAdd(deck2Cards);
  }
